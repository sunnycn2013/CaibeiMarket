...
{
    AFNetworkReachabilityManager *reachablity = [AFNetworkReachabilityManager managerForDomain:@"https://www.baidu.com"];
    AFNetworkReachabilityStatus   status = (int)[reachablity networkReachabilityStatus];
...
+ (id)objectFromData:(NSData *)data
{
    ...
    if (data && [data isKindOfClass:[NSData class]]) {
        @try {
            NSError * error = nil;
            object = [NSJSONSerialization JSONObjectWithData:data options:NSJSONReadingMutableLeaves error:&error];
            if (error) {
                NSLog(@"%@",error);
            }
